...
class UserOrderDetails extends React.Component {
  ...
  convertStatus(status) {
    if (status === "en_attente") {
      this.setState({
        progress: 0,
        time: "Votre commande n'a pas encore été acceptée par le restaurant.",
      });
    } else if (status === "acceptee") {
      this.setState({
        progress: 25,
        time:
          "Votre commande a été acceptée, elle sera prête dans 30 minutes environ.",
      });
    } else if (status === "en_cours") {
      this.setState({
        progress: 50,
        time:
          "Votre commande est en cours de préparation, elle sera prête dans 20 minutes environ.",
      });
      ...
      this.setState({
        progress: 100,
        time:
          "Cette commande a été récupérée au restaurant et est maintenant terminée.",
      });
    }
  }
  ...
  render() {
    ...
    const { dayWeek, day, month, hour, min } = this.convertDate(
      this.state.date
    );
              ...
              <div className="accordion-item--opened accordion-list">
                ...
                <div className="accordion-item__line container">
                  {this.state.items.length === 1 ? (
                    <p>{this.state.items.length} item</p>
                  ) : (
